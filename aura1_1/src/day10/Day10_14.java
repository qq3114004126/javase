...
public class Day10_14 {
	...
	public static void main(String[] args) {
		Tests t = new Tests();
		depositCard d = new depositCard(1002,"ww",1000);
		borrowCard b = new borrowCard(1003,"zl",1500);
		Log1 L = new Log1();
		t.print(L);
...
class Tests{
	void print(bankCard bb) {
		bb.deposit(c);
	}
//	}
}
class Log1 extends bankCard{
...
//	}
	...
	public bankCard(int id,String name,double balance){
		this.id=id;
		this.name=name;
...
//		index++;
	...
	void transfer(double money,bankCard otherCard){
		if(this.balance<money){
			System.out.println("余额不足");
		}else{
			this.balance-=money;
...
class depositCard extends bankCard{
	...
	public depositCard(int id, String name, double balance) {
...
class borrowCard extends bankCard{

	public borrowCard(int id, String name, double balance) {
		super(id, name, balance);
