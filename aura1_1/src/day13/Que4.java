...
/*
 * 
4.	定义一个ProcessArray（数组处理）类，里面有一个process (int[] target,Commond commond)方法（处理方法），其中Commond是个接口，里面有一个抽象方法process(int[] target)。现创建一个数组处理的processArray对象，传入指定的一个数组，通过process方法就可以处理这个指定数组。处理的规则在Commond的实现中实现，（如实现方法中计算数组的总和），分别使用匿名类和lambda表达式来进行测试。——按某种规则处理数组。
 */
//public class Que4 {
//
//}
